...
class Board {
    ...
    public int setOneSolutionConcrete(){
        ...
        while(changeCount > 0){
            ...
            for (int i = squaresTwo.length-1; i >= 0; i--){
                for (int x = 0; x < squaresTwo[i].length; x++){
                    ArrayList<Integer> possible = squaresTwo[i][x].getPossible();
                    if (possible.size() == 1){
                        squaresTwo[i][x].setConcrete(possible.get(0));
                        changeCount += 1;
                    }
                }
            }
    ...
    public int setOneSolution(String isConcrete){
        // is possiblity is only one will set is as the solution and make it unchangable
        // if anything was changed will return true
        int totalChanged = 0;
        int changeCount = 1;
        while(changeCount > 0){
            ...
            for (int i = squaresTwo.length-1; i >= 0; i--){
                for (int x = 0; x < squaresTwo[i].length; x++){
                    ...
                    if (possible.size() == 1){
                        if (isConcrete != ""){
                            squaresTwo[i][x].setValue(possible.get(0));
                        }else{ // it should be concrete
                            squaresTwo[i][x].setConcrete(possible.get(0));
                        }
                        changeCount += 1;
                    }
